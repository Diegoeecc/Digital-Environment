...
for i in range(lenght):
    cont += letras[random.randint(0, 71)]

@app.route("/")
def hello_world():
    return f'<h1>Bienvenido a mi primera página web de python</h1> <a href="/datos-aleatorios"> Ir a datos aleatorios'

@app.route("/datos-aleatorios")
def datosAleatorios():
    return f'<p>{random.choice(lista)}</p> <a href="/"> Regresar </a> <a href="/contr"> Crear Contraseña </a>'

@app.route("/contr")
def contraseñaAleatoria():
    return f'<h1>Tu contraseña es:{cont}</h1>'
